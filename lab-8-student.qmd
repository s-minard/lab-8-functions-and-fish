...
#find the number of rows that have NA (1796)
fish |> 
  mutate(row_has_na = if_any(everything(), is.na)) |> 
  summarise(rows_w_na = sum(row_has_na))

#determine which columns those are in (all in weight)
fish |> 
  summarise(across(everything(), ~ sum(is.na(.)))) |> 
  pivot_longer(everything(), names_to = "variable", values_to = "na_total") |> 
  filter(na_total > 0)
...
# plot for counts each year
fish |> 
  mutate(row_has_na = if_any(everything(), is.na)) |> 
  filter(row_has_na == "TRUE") |> 
  ggplot() +
  ...
  labs(x = "Year",
       ...
       fill = "Section",
       title = "Number of Missing Values Per Year By Section and Trip") +
  theme_bw()
...
> (max(weight, na.rm = TRUE) - min(length, na.rm = TRUE))). Forgot to change length to width in the min() function. 
...
#modification
#must change mutate to return since mutate works with a data frame not a vector
rescale_01 <- function(x) {
  if(!is.numeric(x)) {
    stop("Error: x must be numeric") #if it's not numeric, stop
  }
  if(length(x) <= 1) {
    stop("Error: length not greater than 1")
  }
  return(x = (x - min(x, na.rm = TRUE)) / (max(x, na.rm = TRUE) - (min(x, na.rm = TRUE)))) 
}
...
#create function that takes data frame and returns a new data frame with rescaled variables as specified
...
#rescale length and weight
fish |> 
  rescale_column(length, weight)

#test if it works on just one column at a time (it does)
fish |> 
  rescale_column(weight)
```
